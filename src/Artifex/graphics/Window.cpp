...
Window::Window(const std::string &name, vec<2, uint32_t> size, API api) : size(size) {
  // Decide Window Size
  bool f = (size->width * size->height) == 0;
  ...
  case VULKAN:
    Log::error("Window", "Vulkan support is yet to come!\n");
    break;
