...
uuid_t Renderer::load_shader(const char *vertex, const char *fragment, const char *geometry) {
  ...
  if (strlen(vertex) < minSize || strlen(fragment) < minSize) {
    Log::error("Renderer", "No Shader Resources");
    return 0;
  }
  ...
  if (!success) {
    ...
    Log::error("Renderer", "Failed to Link Shaders:\n%s", infoLog);
    return 0;
  }

  checkErrors();

  Log::verbose("Renderer", "Loaded Shader", infoLog);
...
size_t Renderer::load_texture(const void *data, const vec<2, uint32_t> &size, uint8_t channels) {
  ...
  if (data == nullptr || (size->width == 0 || size->height == 0) || (channels < 1 || channels > 4)) {
    Log::error("Renderer", "Invalid Texture");
    return 0;
  }
  ...
  Log::verbose("Renderer", "Loaded Texture");
