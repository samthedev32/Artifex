...
void Mix::init(Engine *pEngine) {
    ...
    if (!Mix_Init(MIX_INIT_MP3))
        log_warning("Mix::init", "An Error Occured during initialization: %s",
                    Mix_GetError());
    if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048)) {
        log_error("Mix::init", "Failed to open Audio Device: %s",
                  Mix_GetError());
        return;
    }

    if (!Mix_AllocateChannels(16))
        log_warning("Mix::init", "Failed to open more channels: %s",
                    Mix_GetError());
...
void Mix::music(uint16_t id, int loops) {
    if (Mix_PlayMusic(engine->resource.music[id], loops))
        log_warning("Mix::music", "Failed to play music with id %i", id);
}

void Mix::audio(uint16_t id, int channel, int loops) {
    if (Mix_PlayChannel(channel, engine->resource.audio[id], loops))
        log_warning("Mix::audio", "Failed to play audio with id %i", id);
}