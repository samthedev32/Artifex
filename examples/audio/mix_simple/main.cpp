...
int main() {
  ...
  // Load Audio 1
  samples = stb_vorbis_decode_filename("../../../../examples/resources/audio/game.ogg", &channels, &sample_rate, &data);
  uuid_t id1 = mix.load(data, samples, channels == 2 ? Mix::STEREO16 : Mix::MONO16, sample_rate);
  free(data);

  // Load Audio 2
  samples = stb_vorbis_decode_filename("../../../../examples/resources/audio/music.ogg", &channels, &sample_rate, &data);
