...
#include <artifex/core/utility.h>
...
namespace load
{
    ...
    texture rgb(vec3 color)
    {
        ...
        std::string name = "RGB-" + string::itos(color.r * 255) + "-" + string::itos(color.g * 255) + "-" + string::itos(color.b * 255);
    ...
    /* Load Shader (.VS / .FS) */
    shader gls(std::string path, std::string vertex = "", std::string fragment = "")
    {
        shader out;
        ...
        else
        {
            ...
            if (path != "")
            {
                ...
                {
                    LOG::WARNING("load::gls(" + path + ")", "can't open file(s): " + path);
                    return out;
                }
            ...
            if (!success)
            {
                ...
                LOG::WARNING("load::gls(" + path + ")", "can't compile vertex shader: " + std::string(infoLog));
            ...
            if (!success)
            {
                ...
                LOG::WARNING("load::gls(" + path + ")", "can't compile fragment shader: " + std::string(infoLog));
            ...
            if (!success)
            {
                ...
                LOG::WARNING("load::gls(" + path + ")", "can't link shaders: " + std::string(infoLog));
                return out;
            }
    ...
    font ttf(std::string path, int width = 16, int height = 16, int start = 32)
    {
        ...
        else
        {
            ...
            glBufferData(GL_ARRAY_BUFFER, sizeof(out.vertices), out.vertices, GL_STATIC_DRAW);
